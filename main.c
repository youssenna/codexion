...
/*   Updated: 2026/08/06 14:28:49 by yousenna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "codexion.h"

int let_main_wait(t_coder *coders, pthread_t monitor)
{
	int i;
	...
	while (i < coders->prog_info->nb_coders)
	{
		...
		{
			// her i need to stop other threads 
			...
			fprintf(stderr, "System error when trying to join thread number [%d]\n", i + 1);
			return i;
		}
		i++;
	}
...
int destroy_mutex_and_cond(t_progInfo *prog_info, int i)
{
	int ret_mut;
	int ret_cond;
	...
	{
		fprintf(stderr,
			"System error when trying to destroy condition variable of dongle [%d]\n",
			i + 1
		);
		return 0;
	}
...
int clean_resources(t_progInfo *prog_info, int nb_coder)
{
	int	i;
...
int error_handling(t_progInfo *prog_info)
{
	int	check;
	pthread_t monitor;
	...
	if (check != -1)
	{
		// her i need to stop all runing threads
